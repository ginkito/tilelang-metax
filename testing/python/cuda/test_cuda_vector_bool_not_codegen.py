...
def _build_vector_not_source(lanes):
    build = tvm.get_global_func("target.build.tilelang_maca_without_compile", allow_missing=True)
    if build is None:
        pytest.skip("TileLang was built without the MACA code generator")
    return build(_make_vector_not_module(lanes), tvm.target.Target("maca")).inspect_source()
...
def test_vector_not_compiles(lanes):
    build = tvm.get_global_func("target.build.tilelang_maca")
    build(_make_vector_not_module(lanes), tvm.target.Target("maca"))
